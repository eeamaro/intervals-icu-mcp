...
### Added
- **Multi-tenant credentials via per-request query params and HTTP headers.** Over an HTTP transport, each request can carry its own credentials, letting a single deploy serve multiple athletes. Resolution is per request with priority **query params (`api_key` / `athlete_id`) → headers (`X-Intervals-Api-Key` / `X-Intervals-Athlete-Id`) → env-var fallback**; absent values preserve the existing env-var (and stdio) behavior unchanged. Overrides are layered onto `ICUConfig` by `apply_header_credentials()` in `auth.py`, applied in `ConfigMiddleware` (tool calls) and the `intervals-icu://athlete/profile` resource. Backwards compatible (additive). See README *Multi-tenant* and `docs/architecture.md`.
